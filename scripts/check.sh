...
# Tripwire anti-régression Rouge-Gorge — source unique de vérité du "quoi vérifier".
...
# firmware ESP32 à venir — ex.: "*/firmware/*:cd firmware && idf.py build"
MODULE_FAST=()
...
FAST_RUN_CMD="./scripts/kicad-check.sh erc"; FAST_LABEL="Phase rapide"; SCOPE_KEY=""
if [ -n "$CHANGED" ]; then
...
build_variant() {
  ...
  if ( ./scripts/kicad-check.sh drc ) >"$OUTBUF" 2>&1; then
    ok "Build ${v:-complet} OK"
    return 0
  else
    capture_fail "Build ${v:-complet}" "./scripts/kicad-check.sh drc"
    fail "Build ${v:-complet}: échec — détail: $GITDIR/tripwire/last-fail.log (ou relance: ./scripts/kicad-check.sh drc)"
    return 1
  fi
}
